...
if($result_atencion->num_rows==0){//NO EXISTE LA ATENCION PROCEDEMOS A GUARDARLA
	...
	$seguimiento_id = correlativo('seguimiento_id', 'seguimiento');
	$insert = "INSERT INTO seguimiento VALUES('$seguimiento_id','$pacientes_id','$colaborador_id','$servicio_id','$fecha_cita','$motivo_consulta_seguimiento','$hist_enfe_actual_seguimiento','$diagnostico_seguimiento','$manejo_seguimiento','$receta_estudio_seguimiento','$receta_medicamentos_seguimiento','$estado_atencion','$fecha_registro')";
	...
	if($query){
		...
		for($i=0;$i<$totalfiles;$i++){
			...
			if (file_exists($path)){
				...
				$insert = "INSERT INTO seguimiento_detalles VALUES('$seguimiento_detalles_id','$seguimiento_id','$filename','$fecha_registro')";
				$mysqli->query($insert);
			}
		}
